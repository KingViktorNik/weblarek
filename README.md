...
Методы:
  ...
  - `set price(price: number | null)` - установка цены и управление состоянием кнопки.
    - Параметр:
      - `price: number | null` — цена товара или null (если недоступна).
...
Наследование:
  ...
  - Параметр шаблона: `T` — тип данных формы (позволяет специализировать форму для конкретных сценариев).
...
Наследование:
  - Родитель: `Form<IOrderForm>` - Наследует логику управления кнопкой отправки и сообщениями об ошибках;
...
Поля класса:
  - `orderButtons: HTMLElement` - контейнер кнопок выбора способа оплаты. Соответствует селектору `order__buttons`.
  - `buttonsNodes: HTMLButtonElement[]` - список кнопок оплаты. Селектор `button` внутри `order__buttons`.
...
Методы:
  - `set payment(value: TPayment)` - установка выбранного способа оплаты и визуальная индикация.
    - Параметр:
      - `value: TPayment` — значение способа оплаты. Для каждой кнопки в `buttonsNodes`: если `button.name === value`, добавляет класс `button_alt` иначе удаляет класс `button_alt`.
...
Наследование:
  - Родитель: `Form<IContactsForm>` - Наследует логику управления кнопкой отправки и сообщениями об ошибках;
